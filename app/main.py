...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .status-icon { font-size: 5rem; margin-bottom: 12px; display: block; }
        .status-label { font-size: 2.5rem; font-weight: 800; letter-spacing: 0.05em; margin-bottom: 8px; }
        .status-detail { font-size: 1.1rem; opacity: 0.9; }
        ...
        .info-box { background: var(--bg); padding: 12px; border-radius: 8px; text-align: center; }
        .info-val { font-weight: 700; font-size: 1.1rem; display: block; }
        .info-key { font-size: 0.7rem; color: var(--text-sub); }
        ...
        /* Logs */
        .log-list { max-height: 180px; overflow-y: auto; padding: 0 16px; }
        .log-item {
            display: flex; align-items: center; gap: 12px;
            padding: 10px 0; border-bottom: 1px solid var(--border);
            font-size: 0.85rem;
...
<body>
    <div class="container">
        ...
        <div class="grid">
            ...
            <div class="card card-info">
                <div class="card-header">
                    <span class="card-title">📋 最近の履歴</span>
                </div>
                <div class="info-grid">
                    <div class="info-box">
                        <span class="info-val" id="avg-count">--</span>
                        <span class="info-key">本日平均</span>
                    </div>
                    <div class="info-box">
                        <span class="info-val" id="max-count">--</span>
                        <span class="info-key">本日最大</span>
                    </div>
                    <div class="info-box">
                        <span class="info-val" id="total-records">--</span>
                        <span class="info-key">記録数</span>
                    </div>
                </div>
                <div class="log-list" id="log-list">
                    <!-- Logs here -->
    ...
    <script>
        ...
        async function loadStats() {
            try {
                const res = await fetch('/api/crowding/stats?days=1');
                const data = await res.json();

                document.getElementById('avg-count').textContent = data.avg_person_count + '人';
                document.getElementById('max-count').textContent = data.max_person_count + '人';
